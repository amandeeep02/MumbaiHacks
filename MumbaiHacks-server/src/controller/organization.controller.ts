...
export const addOrganization = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
...
export const addEmployee = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!organization) {
    return next(new AppError("Organization not found", 404));
        ...
        if(organization.owner.toString() != userId.toString()) {
                return next(new AppError("User is not the owner of the organization", 403));
        }
